...
//  toturnin
//
//  Created by Alan Nguyen on 3/1/21.
//  Copyright © 2021 Alan Nguyen. All rights reserved.
//
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    {
        Sensor nextItem = s[unsorted]; // problem with s[unsorted] = ?
        int loc = unsorted;
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    {
        Sensor* sensorPtr = &(auxSensors[i]);
        sensorPtrs.push_back(sensorPtr);
    }
