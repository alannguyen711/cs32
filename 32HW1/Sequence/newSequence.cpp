...
// COPY CONSTRUCTOR
Sequence::Sequence (const Sequence &old) // must be const, pass by reference
{
    m_size = old.m_size;
...
Sequence& Sequence::operator=(const Sequence &old)
{
    ...
    m_sequence = new ItemType[maxCapacity];
    for (int i = 0; i < old.m_size; i++)
        insert(i, old.m_sequence[i]);
    return *this;
}
...
{
    int tempSize = m_size; // swap m_size
    m_size = other.m_size;
    other.m_size = tempSize;
    
    int tempMax = maxCapacity; // swap maximum capacities
    maxCapacity = other.maxCapacity;
    other.maxCapacity = tempMax;
    
    ItemType* temporary = m_sequence;
    m_sequence = other.m_sequence;
    other.m_sequence = temporary;
}
